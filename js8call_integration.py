...
class JS8CallClient:
    def __init__(self, driver, logger=None):
        ...
        if self.config.has_section("js8call"):
            # Database is initialized globally in server.py, but we ensure it here too
            initialize_database()
        else:
            self.logger.info(
    ...
    def insert_message(self, table, sender, recipient, message):
        ...
        prebuilt_queries = {
            "messages": "INSERT INTO ham_messages (sender, receiver, message) VALUES (?, ?, ?)",
            "groups": "INSERT INTO ham_groups (sender, groupname, message) VALUES (?, ?, ?)",
            "urgent": "INSERT INTO ham_urgent (sender, groupname, message) VALUES (?, ?, ?)",
        }

        if table not in prebuilt_queries:
            self.logger.error(
                f"Attempted access to invalid/unauthorized table: {table}"
            )
            return
        ...
        try:
            with conn:
                conn.execute(sql, (sender, recipient, message))
    ...
    def process(self, message):
        ...
        if typ == "RX.DIRECTED" and value:
            ...
            if receiver in self.js8urgent:
                ...
                send_message(notification_message, BROADCAST_NUM, self.driver)
            ...
            while self.connected:
                try:
                    ...
                    try:
                        ...
                        self.logger.warning(
                            "Malformed UTF-8 bytes received from JS8Call, skipping chunk"
                        )
                        continue

                    # Safety cap on recv_buffer to prevent memory exhaustion
                    if (
                        len(self.recv_buffer) + len(content.encode("utf-8"))
                        > MAX_RECV_BUFFER
                    ):
                        self.logger.warning(
                            "JS8Call recv_buffer exceeded cap, clearing buffer"
                        )
                    ...
                    while b"\n" in self.recv_buffer:
                        ...
                        try:
                            ...
                            self.logger.warning(
                                f"Invalid JSON content received: {line}. Error: {e}"
                            )
                            continue
...
def handle_js8call_command(sender_id, driver):
    response = (
        "JS8Call Menu:\n[G]roup Messages\n[S]tation Messages\n[U]rgent Messages\nE[X]IT"
    )
...
def handle_js8call_steps(sender_id, message, step, driver, _state):
    ...
    if step == 1:
        ...
        if choice == "x":
            ...
            handle_group_messages_command(sender_id, driver)
        elif choice == "s":
            handle_station_messages_command(sender_id, driver)
        elif choice == "u":
            handle_urgent_messages_command(sender_id, driver)
        else:
            send_message("Invalid option. Please choose again.", sender_id, driver)
            handle_js8call_command(sender_id, driver)


def handle_group_messages_command(sender_id, driver):
    try:
        groups = []
        ...
        if groups:
            ...
            send_message("No group messages available.", sender_id, driver)
            handle_js8call_command(sender_id, driver)
    except Exception:
        logger.exception("Error in handle_group_messages_command")
        send_message("Error retrieving group messages.", sender_id, driver)
        handle_js8call_command(sender_id, driver)


def handle_station_messages_command(sender_id, driver):
    try:
        messages = []
        conn = get_db_connection()
        if not conn:
        ...
        c.execute(
            "SELECT sender, receiver, message, timestamp FROM ham_messages ORDER BY timestamp DESC LIMIT 10"
        )
        messages = c.fetchall()
        ...
        if messages:
            ...
            send_message("No station messages available.", sender_id, driver)
        handle_js8call_command(sender_id, driver)
    except Exception:
        logger.exception("Error in handle_station_messages_command")
        send_message("Error retrieving station messages.", sender_id, driver)
        handle_js8call_command(sender_id, driver)


def handle_urgent_messages_command(sender_id, driver):
    try:
        messages = []
        conn = get_db_connection()
        if not conn:
        ...
        # Order by most recent and limit to prevent oversized responses
        c.execute(
            "SELECT sender, groupname, message, timestamp FROM ham_urgent ORDER BY timestamp DESC LIMIT 10"
        )
        messages = c.fetchall()
        ...
        if messages:
            ...
            send_message("No urgent messages available.", sender_id, driver)
        handle_js8call_command(sender_id, driver)
    except Exception:
        logger.exception("Error in handle_urgent_messages_command")
        send_message("Error retrieving urgent messages.", sender_id, driver)
        handle_js8call_command(sender_id, driver)


def handle_group_message_selection(sender_id, message, _step, state, driver):
    groups = state["groups"]
    try:
        group_index = int(message)
    except ValueError:
        send_message(
            "Invalid input. Please enter a valid group number.", sender_id, driver
        )
        handle_group_messages_command(sender_id, driver)
        return
    ...
    if not (0 <= group_index < len(groups)):
        send_message("Invalid group selection. Please choose again.", sender_id, driver)
        handle_group_messages_command(sender_id, driver)
        return
    ...
    try:
        ...
        # Limit results
        c.execute(
            "SELECT sender, message, timestamp FROM ham_groups WHERE groupname=? ORDER BY timestamp DESC LIMIT 10",
            (groupname,),
        )
        messages = c.fetchall()
        ...
        if messages:
            ...
            send_message(f"No messages for group {groupname}.", sender_id, driver)

        handle_js8call_command(sender_id, driver)
    except Exception:
        logger.exception("Error fetching group messages")
        send_message("An error occurred while fetching messages.", sender_id, driver)
        handle_group_messages_command(sender_id, driver)

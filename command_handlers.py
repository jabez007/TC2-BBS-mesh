...
def handle_help_command(sender_id, driver, menu_name=None):
    if menu_name:
...
def handle_mail_command(sender_id, driver):
...
def handle_exit_command(sender_id, driver):
    send_message("Type 'HELP' for a list of commands.", sender_id, driver)
    update_user_state(sender_id, None)


def handle_stats_command(sender_id, driver):
    response = "📊Stats Menu📊\nWhat stats would you like to view?\n[N]odes  [H]ardware  [R]oles  E[X]IT"
    send_message(response, sender_id, driver)
    update_user_state(sender_id, {'command': 'STATS', 'step': 1})


def handle_fortune_command(sender_id, driver):
    try:
        with open('fortunes.txt', 'r') as file:
            fortunes = file.readlines()
        if not fortunes:
            send_message("No fortunes available.", sender_id, driver)
            return
...
def handle_bb_steps(sender_id, message, step, state, driver, bbs_nodes):
    ...
    if step == 1:
        ...
        if message.lower() == 'r':
            bulletins = get_bulletins(board_name)
            if bulletins:
                send_message(f"Select a bulletin number to view from {board_name}:", sender_id, driver)
                for bulletin in bulletins:
                    send_message(f"[{bulletin[0]}] {bulletin[1]}", sender_id, driver)
                update_user_state(sender_id, {'command': 'BULLETIN_READ', 'step': 3, 'board': board_name})
            else:
                send_message(f"No bulletins in {board_name}.", sender_id, driver)
            ...
            if board_name.lower() == 'urgent':
                ...
                if allowed_nodes and node_id not in allowed_nodes:
                    ...
                    handle_bb_steps(sender_id, 'e', 1, state, driver, bbs_nodes)
                    return
            send_message("What is the subject of your bulletin? Keep it short.", sender_id, driver)
        ...
        subject = message
        send_message("Send the contents of your bulletin. Send a message with END when finished.", sender_id, driver)
...
def handle_mail_steps(sender_id, message, step, state, driver, bbs_nodes):
    ...
    if step == 1:
        ...
        if choice == 'r':
            ...
            if mail:
                send_message(f"You have {len(mail)} mail messages. Select a message number to read:", sender_id, driver)
                for msg in mail:
                    send_message(f"-{msg[0]}-\nDate: {msg[3]}\nFrom: {msg[1]}\nSubject: {msg[2]}", sender_id, driver)
                update_user_state(sender_id, {'command': 'MAIL', 'step': 2})
            else:
                send_message("There are no messages in your mailbox.📭", sender_id, driver)
                update_user_state(sender_id, None)
        elif choice == 's':
            send_message("What is the Short Name of the node you want to leave a message for?", sender_id, driver)
        ...
        nodes = get_node_info(driver, short_name)
        if not nodes:
            ...
            send_message("There are multiple nodes with that short name. Which one would you like to leave a message for?", sender_id, driver)
            for i, node in enumerate(nodes):
                send_message(f"[{i}] {node['longName']}", sender_id, driver)
        ...
        if message.lower() == "d":
            ...
            sender = state['sender']
            send_message(f"Send your reply to {sender} now, followed by a message with END", sender_id, driver)
            update_user_state(sender_id, {'command': 'MAIL', 'step': 7, 'reply_to_mail_id': state['mail_id'], 'subject': f"Re: {state['subject']}", 'content': ''})
        else:
            send_message("The message has been kept in your inbox.✉️", sender_id, driver)
            update_user_state(sender_id, None)

    elif step == 5:
        subject = message
        send_message("Send your message. You can send it in multiple messages if it's too long for one.\nSend a single message with END when you're done", sender_id, driver)
        ...
        if message.lower() == "y":
            ...
            send_message("Okay, feel free to send another command.", sender_id, driver)
            update_user_state(sender_id, None)
...
def handle_channel_directory_command(sender_id, driver):
    response = "📚CHANNEL DIRECTORY📚\nWhat would you like to do?\n[V]iew  [P]ost  E[X]IT"
    send_message(response, sender_id, driver)
    update_user_state(sender_id, {'command': 'CHANNEL_DIRECTORY', 'step': 1})


def handle_channel_directory_steps(sender_id, message, step, state, driver):
    message = message.strip()
    if len(message) == 2 and message[1] == 'x':
        message = message[0]
    ...
    if step == 1:
        ...
        if choice.lower() == 'x':
            ...
            if channels:
                ...
                send_message("No channels available in the directory.", sender_id, driver)
                handle_channel_directory_command(sender_id, driver)
        elif choice.lower() == 'p':
            send_message("Name your channel for the directory:", sender_id, driver)
        ...
        send_message("Send a message with your channel URL or PSK:", sender_id, driver)
...
def handle_send_mail_command(sender_id, message, driver, bbs_nodes):
    try:
        parts = message.split(",,", 3)
        if len(parts) != 4:
            send_message("Send Mail Quick Command format:\nSM,,{short_name},,{subject},,{message}", sender_id, driver)
            return

        _, short_name, subject, content = parts
        nodes = get_node_info(driver, short_name.lower())
        if not nodes:
            send_message(f"Node with short name '{short_name}' not found.", sender_id, driver)
            return
        if len(nodes) > 1:
        ...
        send_message("Error processing send mail command.", sender_id, driver)


def handle_check_mail_command(sender_id, driver):
    try:
        sender_node_id = get_node_id_from_num(sender_id, driver)
        mail = get_mail(sender_node_id)
        if not mail:
            send_message("You have no new messages.", sender_id, driver)
            return
        ...
        send_message("Error processing check mail command.", sender_id, driver)


def handle_read_mail_command(sender_id, message, state, driver):
    try:
        mail = state.get('mail', [])
        message_number = int(message) - 1

        if message_number < 0 or message_number >= len(mail):
            send_message("Invalid message number. Please try again.", sender_id, driver)
            return
        ...
        send_message("Error processing read mail command.", sender_id, driver)


def handle_delete_mail_confirmation(sender_id, message, state, driver, bbs_nodes):
    try:
        ...
        send_message("Error processing delete mail confirmation.", sender_id, driver)



def handle_post_bulletin_command(sender_id, message, driver, bbs_nodes):
    try:
        parts = message.split(",,", 3)
        if len(parts) != 4:
            send_message("Post Bulletin Quick Command format:\nPB,,{board_name},,{subject},,{content}", sender_id, driver)
            return
        ...
        send_message("Error processing post bulletin command.", sender_id, driver)


def handle_check_bulletin_command(sender_id, message, driver):
    try:
        # Split the message only once
        parts = message.split(",,", 1)
        if len(parts) != 2 or not parts[1].strip():
            send_message("Check Bulletins Quick Command format:\nCB,,board_name", sender_id, driver)
            return
        ...
        if not board_name:
            ...
            send_message(f"Board '{board_name_input}' not found. Valid boards: {valid_boards}", sender_id, driver)
            return

        bulletins = get_bulletins(board_name)
        if not bulletins:
            send_message(f"No bulletins available on {board_name} board.", sender_id, driver)
            return
        ...
        send_message("Error processing check bulletin command.", sender_id, driver)

def handle_read_bulletin_command(sender_id, message, state, driver):
    try:
        bulletins = state.get('bulletins', [])
        message_number = int(message) - 1

        if message_number < 0 or message_number >= len(bulletins):
            send_message("Invalid bulletin number. Please try again.", sender_id, driver)
            return
        ...
        send_message("Invalid input. Please enter a valid bulletin number.", sender_id, driver)
    except Exception:
        logger.exception("Error processing read bulletin command")
        send_message("Error processing read bulletin command.", sender_id, driver)
...
def handle_post_channel_command(sender_id, message, driver):
    try:
        ...
        send_message("Error processing post channel command.", sender_id, driver)


def handle_check_channel_command(sender_id, driver):
    try:
        channels = get_channels()
        if not channels:
            send_message("No channels available in the directory.", sender_id, driver)
        ...
        send_message("Error processing check channel command.", sender_id, driver)


def handle_read_channel_command(sender_id, message, state, driver):
    try:
        channels = state.get('channels', [])
        message_number = int(message) - 1

        if message_number < 0 or message_number >= len(channels):
            send_message("Invalid channel number. Please try again.", sender_id, driver)
            return
        ...
        send_message("Invalid input. Please enter a valid channel number.", sender_id, driver)
    except Exception:
        logger.exception("Error processing read channel command")
        send_message("Error processing read channel command.", sender_id, driver)


def handle_list_channels_command(sender_id, driver):
    try:
        channels = get_channels()
        if not channels:
            send_message("No channels available in the directory.", sender_id, driver)
        ...
        send_message("Error processing list channels command.", sender_id, driver)


def handle_quick_help_command(sender_id, driver):
